...
public interface PostDao {
	...
	@Select("select u.user_nickname, p.img_id, p.post_no, p.board_no, p.user_id, p.title, p.p_content, p.regist_date, p.update_date, p.keyword, p.location, p.hit, p.img_url, "
			...
			+ "h on p.post_no = h.post_no where p.post_no = #{post_no} order by post_no DESC")
	...
	//* 검색기능 new */
   @Select("SELECT u.user_nickname, p.img_id, p.post_no, p.board_no, p.user_id, p.title, p.p_content, p.regist_date, p.update_date, p.keyword, p.location, p.hit, p.img_url, "
           + "COALESCE(h.like_cnt, 0) like_cnt FROM post p "
           + "JOIN userinfo u ON p.user_id = u.user_id "
           + "LEFT OUTER JOIN (SELECT COUNT(*) like_cnt, post_no FROM heart GROUP BY post_no) h ON p.post_no = h.post_no "
           + "WHERE p.board_no = #{board_no} AND (p.title LIKE CONCAT('%', #{keyword}, '%') OR p.p_content LIKE CONCAT('%', #{keyword}, '%'))")
   public List<PostDto> TotalPostSearchList(@Param("board_no") int board_no, @Param("keyword") String keyword) throws DataAccessException;
	...
	@Select("SELECT * FROM reply WHERE reply_no = #{reply_no} desc")
